...
export async function downloadStockfish() {
  ...
  try {
    ...
    const extractedDir = existsSync(resolve(tmp, 'stockfish')) ? resolve(tmp, 'stockfish') : tmp;
    mkdirSync(resolve('stockfish'), { recursive: true });
    cpSync(extractedDir, resolve('stockfish'), { recursive: true });
    ...
    const binary = findBinary(resolve('stockfish'));
    if (!binary) throw new Error(`${target.binName} not found inside downloaded archive`);

    if (!existsSync(targetBin) && binary !== targetBin) {
      try {
        copyFileSync(binary, targetBin);
      } catch {}
    }
    if (!existsSync(genericBin) && targetBin !== genericBin) {
      try {
        copyFileSync(binary, genericBin);
      } catch {}
    }

    if (process.platform !== 'win32') {
      if (existsSync(targetBin)) chmodSync(targetBin, 0o755);
      if (existsSync(genericBin)) chmodSync(genericBin, 0o755);
      if (existsSync(binary)) chmodSync(binary, 0o755);
    }
    console.log(`Stockfish package installed with all files at ${resolve('stockfish')}`);
  } finally {
